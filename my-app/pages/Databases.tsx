   ...
   // console.log(supabase)
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            const {data, error} = await supabase
                ...
                */
               if(HomesError) setFetchErrors(prevErrors => ({...prevErrors, Homes: 'Could not fetch Homes data'}))
               if(RealtorError) setFetchErrors(prevErrors => ({...prevErrors, Realtor: 'Could not fetch Realtor data'}))
               if(UserInfoError) setFetchErrors(prevErrors => ({...prevErrors, UserInformation: 'Could not fetch User Information data'}))
    ...
    return (
        <div className="page home">
            {Object.keys(fetchErrors).map((table, index) => (
                <p key={index}>{fetchErrors[table]}</p>
            ))}
