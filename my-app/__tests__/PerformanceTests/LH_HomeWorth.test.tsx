...
import fs from 'fs';
import { AuditRef, LighthouseReport } from '../types'; // Ensure correct path to types

const path = './__tests__/PerformanceTests/reports/homeworth-page-report.json';

describe('HomeWorth (Valuation) Page Performance Tests', () => {

  // 1. Run Lighthouse performance test for the HomeWorth (Valuation) page
  it('should run Lighthouse performance test for the HomeWorth (Valuation) page', (done: jest.DoneCallback) => {
    exec(
      'lighthouse http://localhost:3000/valuation --output=json --output-path=./__tests__/PerformanceTests/reports/homeworth-page-report.json',
      (err, stdout, stderr) => {
        if (err) {
