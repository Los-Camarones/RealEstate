...
export async function GET(req: Request) {
  ...
  if (!id) {
    return NextResponse.json({ error: 'Missing more info request ID' }, { status: 400 });
  }

  try {
    // Fetch the specific listing report signup request with the given ID
    const response = await axios.get(`https://www.idxhome.com/api/v1/client/moreInfoRequest/${id}.json`, {
      headers: {
        Authorization: token,
    ...
    console.error('Error fetching more info report request:', error.response?.data || error.message);
    return NextResponse.json({ error: 'Failed to fetch more info report request' }, { status: 500 });
  }
}
