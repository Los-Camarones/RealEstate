...
export async function GET(req: Request) {
  ...
  if (!id) {
    return NextResponse.json({ error: 'Missing listing report signup request ID' }, { status: 400 });
  }

  try {
    // Fetch the specific listing report signup request with the given ID
    const response = await axios.get(`https://www.idxhome.com/api/v1/client/listingReportSignupRequest/${id}.json`, {
      headers: {
        Authorization: token,
