...
export async function GET(req: Request) {
  ...
  try {
    ...
    if (id) {
      ...
      API_URL = `https://www.idxhome.com/api/v1/client/listingReportSignupRequest/${id}.json`;
    } else {
      ...
      API_URL = `https://www.idxhome.com/api/v1/client/listingReportSignupRequests.json?fields=${fields}&offset=${offset}&limit=${limit}`;
    }
    ...
    console.error('Error fetching listing report signup requests:', error.response?.data || error.message);
    return NextResponse.json({ error: 'Failed to fetch listing report signup requests' }, { status: 500 });
  }
}
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const response = await axios.post(`https://www.idxhome.com/api/v1/client/listingReportSignupRequests.json`, {
